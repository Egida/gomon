...
Internally `gomon` caps destination tracking at 1024 unique endpoints per window to keep memory usage predictable; windows that exceed the cap log a warning and continue with the sampled set.
...
- `dst_ips` enumerates every unique destination contacted in the window (IP, port, protocol).
